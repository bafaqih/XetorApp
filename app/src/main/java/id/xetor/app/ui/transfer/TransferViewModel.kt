// app/src/main/java/id/xetor/app/ui/transfer/TransferViewModel.kt
...
class TransferViewModel(
    ...
    init {
        ...
        loadTransferData(showLoading = true)
    }

    /**
     * Load transfer data dengan loading skeleton
     * Digunakan saat initial load atau manual refresh
     */
    fun loadTransferData(showLoading: Boolean = true) {
        viewModelScope.launch {
            ...
            if (walletResult.isSuccess && transactionsResult.isSuccess) {
                ...
                val transferTransactions = allTransactions.filter { it.type == "transfer" }
                
                _uiState.update {
                    it.copy(
                        isLoading = false,
                        wallet = walletResult.getOrNull(),
                        allTransactions = transferTransactions,
    ...
    fun silentRefresh() {
        ...
        if (lastRefreshTime == 0L || timeSinceLastRefresh >= REFRESH_INTERVAL_MS) {
            android.util.Log.d("TransferViewModel", "Silent refresh triggered (time since last: ${timeSinceLastRefresh}ms)")
            loadTransferData(showLoading = false)
        } else {
            android.util.Log.d("TransferViewModel", "Skip silent refresh (data masih fresh, time since last: ${timeSinceLastRefresh}ms)")
        }
    }

    fun forceRefresh() {
        android.util.Log.d("TransferViewModel", "Force refresh triggered (ignore interval)")
        loadTransferData(showLoading = false)
    }
    ...
    fun submitTransfer(
        ...
        viewModelScope.launch {
            val result = userRepository.submitTransfer(
                token = token,
                amount = amount,
            ...
            if (result.isSuccess) {
                ...
                val errorMsg = result.exceptionOrNull()?.message ?: "Gagal melakukan transfer"
                // Map backend error messages to user-friendly messages
                val userFriendlyMsg = when {
                    errorMsg.contains("tidak ditemukan", ignoreCase = true) || 
                    errorMsg.contains("not found", ignoreCase = true) -> "Akun penerima tidak ditemukan"
                    else -> errorMsg
                }
