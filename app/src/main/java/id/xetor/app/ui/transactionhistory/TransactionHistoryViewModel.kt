// app/src/main/java/id/xetor/app/ui/transactionhistory/TransactionHistoryViewModel.kt
...
// State untuk UI Transaction History
...
class TransactionHistoryViewModel(
    ...
    init {
        ...
        loadTransactionHistory(showLoading = true)
    }

    /**
     * Load transaction history dengan loading skeleton
     * Digunakan saat initial load atau manual refresh
     */
    fun loadTransactionHistory(showLoading: Boolean = true) {
    ...
    private fun filterTransactions(
        ...
        return transactions.filter { transaction ->
            ...
            val matchesType = if (transactionType.value == null) {
                true
            } else {
            ...
            matchesType && isWithinDateRange && matchesStatus
        }
    }
     ...
     * Digunakan saat kembali ke transaction history screen
     */
    fun silentRefresh() {
        ...
        if (lastRefreshTime == 0L || timeSinceLastRefresh >= REFRESH_INTERVAL_MS) {
            android.util.Log.d("TransactionHistoryViewModel", "Silent refresh triggered (time since last: ${timeSinceLastRefresh}ms)")
            loadTransactionHistory(showLoading = false)
        } else {
            android.util.Log.d("TransactionHistoryViewModel", "Skip silent refresh (data masih fresh, time since last: ${timeSinceLastRefresh}ms)")
        }
    }

    /**
     * Force refresh tanpa loading skeleton, selalu refresh tanpa cek interval
     * Digunakan saat transaksi berhasil untuk langsung update data
     */
    fun forceRefresh() {
        android.util.Log.d("TransactionHistoryViewModel", "Force refresh triggered (ignore interval)")
        loadTransactionHistory(showLoading = false)
    }
}
